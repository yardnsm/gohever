// Code generated by mockery v2.14.0. DO NOT EDIT.

package mocks

import mock "github.com/stretchr/testify/mock"

// RequestHandler is an autogenerated mock type for the requestHandler type
type RequestHandler[T interface{}] struct {
	mock.Mock
}

// Execute provides a mock function with given fields:
func (_m *RequestHandler[T]) Execute() (*T, error) {
	ret := _m.Called()

	var r0 *T
	if rf, ok := ret.Get(0).(func() *T); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*T)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func() error); ok {
		r1 = rf()
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

type mockConstructorTestingTNewRequestHandler interface {
	mock.TestingT
	Cleanup(func())
}

// NewRequestHandler creates a new instance of RequestHandler. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewRequestHandler[T interface{}](t mockConstructorTestingTNewRequestHandler) *RequestHandler[T] {
	mock := &RequestHandler[T]{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
